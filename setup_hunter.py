...
def build_strategies_keyboard(chat_id):
    s = get_user_settings(chat_id)
    enabled = set(s.get("enabled_strategies") or [])
    rows = [[{"text": "Сетапы (вкл / выкл)", "callback_data": "noop"}]]
    for sid, title in CORE_STRATEGY_BUTTONS:
        on = sid in enabled
        mark = "✅" if on else "⬜"
        action = "вкл" if on else "выкл"
        rows.append([{
            "text": f"{mark} {title}",
            "callback_data": f"strat:{sid}",
        }])
...
def handle_callback(callback_query):
    ...
    if data == "setup_back":
        ...
        sid = data.split(":", 1)[1]
        if sid in dict(CORE_STRATEGY_BUTTONS):
